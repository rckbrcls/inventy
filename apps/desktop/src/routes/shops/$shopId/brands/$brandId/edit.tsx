...
import { BrandsRepository, UpdateBrandDTO } from "@/lib/db/repositories/brands-repository"

export const Route = createFileRoute("/shops/$shopId/brands/$brandId/edit")({
  component: EditBrand,
})
...
function EditBrand() {
  ...
  React.useEffect(() => {
    const loadBrand = async () => {
      try {
        setIsLoading(true)
        const brand = await BrandsRepository.getById(brandId)
        if (!brand) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h3 className="text-lg font-medium">Edit Brand</h3>
        <p className="text-sm text-muted-foreground">
          Update brand information and save your changes.
        </p>
      </div>
