...
import { CheckoutsRepository, UpdateCheckoutDTO } from "@/lib/db/repositories/checkouts-repository"

export const Route = createFileRoute("/shops/$shopId/checkouts/$checkoutId/edit")({
...
function EditCheckout() {
  ...
  const [checkoutToken, setCheckoutToken] = React.useState<string | null>(null)
  ...
  React.useEffect(() => {
    const loadCheckout = async () => {
      try {
        setIsLoading(true)
        const checkout = await CheckoutsRepository.getById(checkoutId)
        if (!checkout) {
          toast.error("Checkout not found")
          navigate({ to: "/checkouts" })
          return
        }
        setCheckoutToken(checkout.token || null)
        setFormData({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h3 className="text-lg font-medium">Edit Checkout</h3>
        <p className="text-sm text-muted-foreground">
          Update checkout information. Token:{" "}
          <span className="font-mono text-xs">{checkoutToken || "-"}</span>
        </p>
      </div>
